...
labs = np.zeros((image_numbers, 101), dtype=np.int)
labs_index = 0
for x in labs_mid:
...
def cnnLayer():
    ...
    Wout = tf.get_variable("Wout", shape=[512, 101], initializer=tf.random_normal_initializer(stddev=0.01))
    #bout = biasVariable([2])
    bout = tf.get_variable("bout", shape=[101], initializer=tf.random_normal_initializer)
