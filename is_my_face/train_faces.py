...
imgs = []
labs = []
#下面注释部分不需要了因为之前get_my_faces与set_other_faces都做了
...
def cnnLayer():
    ...
    Wout = tf.get_variable("Wout", shape=[512, 2], initializer=tf.random_normal_initializer(stddev=0.01))
    #bout = biasVariable([2])
    bout = tf.get_variable("bout", shape=[2], initializer=tf.random_normal_initializer)
